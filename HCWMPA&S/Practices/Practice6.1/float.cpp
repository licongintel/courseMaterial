...
#define ORIGINAL 16
float original[ORIGINAL] = {0.5, 1.0, 1.368, 1.394, 
...
int main()
{
    ...
    {
        td[t].thread_id = t;
        td[t].x = new float[SERIES_T];
        td[t].mu = new float[SERIES_T];
        for (int i = 0; i < SERIES_T; i++)
        {
            td[t].x[i] = 0;
        ...
        td[t].records = new float*[RECORD_NUM];
        for (int i = 0; i < RECORD_NUM; i++)
        {
            td[t].records[i] = new float[SERIES_T];
        }
    }
    ...
    for (int i = 0; i < RECORD_NUM; i++)
    {
        ...
        {
            printf("%6.3f ", td[t].records[i][group * ORIGINAL + j]);
        }
        printf("\n");
    }
    return 0;
