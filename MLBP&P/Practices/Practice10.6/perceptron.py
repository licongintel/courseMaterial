...
class Perceptron(object):
    ...
    def train(self, training_data):
        if self.kernel != None:
            matrix = kernels.GramMatrix(training_data,
                    self.kernel)
            ...
            if label not in self.binary_perceptrons:
                ...
                if self.kernel is None:
                    self.binary_perceptrons[label] =\
                            BinaryPerceptron(self.\
                            max_iteration, self.margin)
                else:
                    self.binary_perceptrons[label] =\
                            BinaryKernelPerceptron(self.\
                            kernel, self.max_iteration,
                            self.margin)  
    ...
    def classify(self, test_sample):
        kernel_result = None
        if self.kernel != None:
            kernel_result = self.get_kernel_result(test_sample)        
        ...
        for label in self.binary_perceptrons.keys():
            ...
            score = self.binary_perceptrons[label].\
                    score(test_sample, kernel_result)                    
    ...
    def get_kernel_result(self, sample):
        result = numpy.zeros(self.training_data.get_data_size())
        for i in range(0, len(result)):
            result[i] = self.kernel.calculate(
                    self.training_data.get_feature(i), sample)
        return result
...
class BinaryKernelPerceptron(object):
    ...
    def score_row(self, row):
        return numpy.dot(self.c, row) + self.b

    def train(self, training_data, matrix = None):
        if matrix is None:
            matrix = kernels.GramMatrix(training_data, self.kernel)
        ...
        self.max_norm_sqr = matrix.get_max_norm_sqr()
        self.threshold = self.max_norm_sqr * self.margin
        ...
        for i in range(0, self.max_iteration):
            ...
            for j in range(0, training_data.get_data_size()):
                ...
                score = self.score_row(matrix.get_row(j))
    ...
    def score(self, test_sample, kernel_result = None):
        if kernel_result is not None:
            return self.score_row(kernel_result)
